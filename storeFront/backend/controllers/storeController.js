...
module.exports.addProduct = async (req, res) => {
  ...
  product.m_ts_shortSleeve = req.body.shortSleeve;
  product.m_ts_longSleeve = req.body.longSleeve;
  product.m_ts_performance = req.body.performance;
  product.m_sh_shortSleeve = req.body.shortSleeve;
  product.m_sh_longSleeve = req.body.longSleeve;
  product.m_ts_performance = req.body.performance;
  product.m_po_shortSleeve = req.body.shortSleeve;
  product.m_po_longSleeve = req.body.longSleeve;
  product.w_ts_shortSleeve = req.body.shortSleeve;
  product.w_ts_longSleeve = req.body.longSleeve;
  product.w_ts_performance = req.body.performance;
  product.w_sh_shortSleeve = req.body.shortSleeve;
  product.w_sh_longSleeve = req.body.longSleeve;
  product.w_po_shortSleeve = req.body.shortSleeve;
  product.w_po_longSleeve = req.body.longSleeve;
...
module.exports.getProductMen = async (req, res) => {
  try {
    let response = await Product.find({men : "true"}).sort({created_at: -1});
    if (response.err)
      return res
        .status(404)
        .json({ status: false, message: "Failed to retrive details" });
...
module.exports.getProductWomen = async (req, res) => {
  try {
    let response = await Product.find({women : "true"}).sort({created_at: -1});
    if (response.err)
      return res
        .status(404)
        .json({ status: false, message: "Failed to retrive details" });
